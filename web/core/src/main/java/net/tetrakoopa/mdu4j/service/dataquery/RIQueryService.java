...
import net.tetrakoopa.mdu4j.service.dataquery.model.Column;
import net.tetrakoopa.mdu4j.service.dataquery.model.Entity;
...
public class RIQueryService implements DataQueryService {
	...
	private final static String SERIALIZED_RESOURCE_OPERATORS = "net/tetrakoopa/mdu4j/service/dataquery/operators.json";
	...
	public Schema buildSchema(String name, Class<?>... entityClasses) {
		...
		if (fakeItAnymway) {
			try {
				return new Gson().fromJson(IOUtil.readString("net/tetrakoopa/mdu4j/service/dataquery/schema.json"), Schema.class);
			} catch (IOException e) {
	...
	public List<AggregateFunction> createAggregateFunctions() {
		try {
			return new Gson().fromJson(IOUtil.readString("net/tetrakoopa/mdu4j/service/dataquery/aggregateFunctions.json"), List.class);
		} catch (IOException e) {
			throw new RuntimeException(e);
		}
	}
