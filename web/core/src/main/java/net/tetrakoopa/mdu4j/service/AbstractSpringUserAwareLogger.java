...
public abstract class AbstractSpringUserAwareLogger implements Logger {
	...
	public AbstractSpringUserAwareLogger(String parentName) {
		this(LoggerFactory.getLogger(parentName));
	}
	public AbstractSpringUserAwareLogger(Class<?> parentClassName) {
		this(LoggerFactory.getLogger(parentClassName));
	}
    public AbstractSpringUserAwareLogger(Logger parent) {
        this.parent = parent;
    }

    protected abstract String getUserName(Authentication authentication);

    private String getUserNameOrDefault(Authentication authentication) {
        try {
            return getUserName(authentication);
        } catch (Exception ex) {
            return "<Error:"+ex.getClass()+":"+ex.getMessage()+">";
        }
    }
